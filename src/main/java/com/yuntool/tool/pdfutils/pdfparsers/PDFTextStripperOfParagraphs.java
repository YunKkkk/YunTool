 ...
 */
@Data
public class PDFTextStripperOfParagraphs extends MyPDFTextStripper {
    //段落
    ...
    public PDFTextStripperOfParagraphs() throws Exception {
        super();
    }
...
//        System.out.println(fontSize);
     ...
     * 判断模式
     */
    private void isBigTitleMode(float fontSize) {
        if (previousStr.equals("日")) {
            //开始正文
            startMainBody = true;
        }
    ...
    public List<Paragraphs> getParagraphs() {
        ...
        return paragraphs.stream()
                .filter(p -> {
                    ...
                    return !(title.contains("内部资料供领导参考") || pattern1.matcher(title).find());
                })
